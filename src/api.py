"""FastAPI application for edge-ai-inference-engine."""
...
app = FastAPI(title="edge-ai-inference-engine", version="0.1.0", description="edge-ai-inference-engine API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "edge-ai-inference-engine", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "edge-ai-inference-engine", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "edge-ai-inference-engine", "version": "0.1.0", "ready": True,
